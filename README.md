...
`nvnmchain-anchoring sync` copies what is new and exits. Both refuse an index built from another
chain or contract, since every chain numbers its registries from 1; delete `DB_PATH` to rebuild it.
...
| `POLL_SECONDS` | `2` | how often to look for new registries; a fraction is fine, 0.1 at least |
...
2,182 of them. The chain refused a contains query under three characters, which its trigram index
could not serve; a scan can, so this refuses none.

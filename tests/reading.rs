...
fn a_records_versions_come_back_in_log_order() {
    ...
    // Who wrote a version, and what it claimed to be, exist only in its own
    // envelope -- the listing carries the newest version's and nothing else does.
    assert!(
        versions.iter().all(|v| v.author.starts_with("0x")),
        "who wrote a version is in its own envelope and nowhere else"
    );
}
